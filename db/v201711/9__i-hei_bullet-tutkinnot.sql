...
delete from hei_bullet where mittariryhma='koulutus' and mittari='tutkinnot';
...
insert into hei_bullet (oppilaitos,nimi,nimi_sv,nimi_en,vuosi,mittariryhma,mittari,arvo,tavoite,minimi,raja1,raja2,maksimi)
  ...
  ,'tutkinnot' as mittari
  ,arvo
  ,null as tavoite
  ,0 as minimi
  ,(select percentile_cont(.33) within group (order by t2.arvo) from up.t_vipunen_tutkinnot t2 where t2.vuosi=t.vuosi)  as raja1
  ,(select percentile_cont(.67) within group (order by t2.arvo) from up.t_vipunen_tutkinnot t2 where t2.vuosi=t.vuosi)  as raja2
  ,(select max(t2.arvo)*1.1 from up.t_vipunen_tutkinnot t2 where t2.vuosi=t.vuosi) as maksimi --nb! +10%
from up.t_vipunen_tutkinnot as t
